...
function buildModeRules(mode: Mode, lang: Lang, state: SessionState) {
  const common =
    lang === "ko"
      ? ["응답 길이 규칙:", "- 기본 3~5문장 이내.", "- 불릿 최대 3개.", "- 결론 먼저, 이유는 짧게."].join("\n")
      : ["Response length rules:", "- Default: 3–5 sentences.", "- Bullets: max 3.", "- Lead with the conclusion; keep rationale short."].join("\n");
...
function buildCharacterSystemPrompt(body: Body, state: SessionState) {
  ...
  lines.push(lang === "ko" ? `너는 "${ch?.title}"다. 태그라인: ${ch?.tagline}` : `You are "${ch?.title}". Tagline: ${ch?.tagline}`);
...
function buildInput(body: Body, state: SessionState) {
  return [
    { role: "system" as const, content: buildCharacterSystemPrompt(body, state) },
...
async function tryOpenAIStreamToSSE(res: Response, input: any, signal: AbortSignal) {
  ...
  try {
    ...
    if (!sentAnyToken) return { ok: false as const, reason: "OpenAI stream produced no visible text tokens" };

    sendEvent(res, "done", {});
    return { ok: true as const };
  } catch (e: any) {
    clearTimeout(headerTimer);
    if (e?.name === "AbortError") return { ok: false as const, reason: "stream attempt aborted" };
...
function nextQuestionForMode(mode: Mode, lang: Lang, state: SessionState): string | null {
  if (lang === "ko") {
    switch (mode) {
      case "profile": return "좋아요. 어느 정도로 볼까요?\n1) 요약\n2) 자세히";
      case "fortune": return "좋아요. 기간은 어느 쪽이야?\n1) 이번 주\n2) 이번 달\n3) 2026년 상반기";
      case "career": return "좋아요. 지금 목표는 뭐야?\n1) 취업/지원\n2) 레주메\n3) 면접\n4) 포트폴리오";
      case "love": return "좋아요. 상황이 어디에 가까워?\n1) 썸/대화 중\n2) 연애 중\n3) 장기 관계\n4) 이별/정리 중";
      case "chat":
      default: return "좋아. 지금 가장 신경 쓰이는 한 가지가 뭐야?";
    }
  }
  switch (mode) {
    case "profile": return "Got it. How detailed do you want it?\n1) Summary\n2) Detailed";
    case "fortune": return "Got it. What timeframe?\n1) This week\n2) This month\n3) 2026 (first half)";
    case "career": return "Got it. What’s your main goal?\n1) Job search\n2) Resume\n3) Interviews\n4) Portfolio";
    case "love": return "Got it. Which best describes your situation?\n1) Talking stage\n2) Dating\n3) Long-term\n4) Breakup / moving on";
    case "chat":
    default: return "Okay—what’s the one thing on your mind right now?";
  }
}
...
function handleSlotFill(state: SessionState, body: Body): { handled: boolean; reply?: string } {
  ...
  if (mode === "profile") {
    if (!state.slots?.profile?.depth) {
      if (!choice || (choice !== 1 && choice !== 2)) {
        return { handled: true, reply: lang === "ko" ? "번호로 골라줘.\n1) 요약\n2) 자세히" : "Please choose a number.\n1) Summary\n2) Detailed" };
      }
  ...
  if (mode === "fortune") {
    ...
    if (!f.timeframe) {
      if (!choice || (choice !== 1 && choice !== 2 && choice !== 3)) {
        return { handled: true, reply: lang === "ko" ? "기간을 번호로 골라줘.\n1) 이번 주\n2) 이번 달\n3) 2026년 상반기" : "Choose a timeframe.\n1) This week\n2) This month\n3) 2026 (first half)" };
      }
      f.timeframe = choice === 1 ? "week" : choice === 2 ? "month" : "half_year";
      state.slots!.fortune = f;
      return { handled: true, reply: lang === "ko" ? "주제는 뭐로 볼까?\n1) 커리어\n2) 연애\n3) 돈\n4) 건강" : "What topic should we focus on?\n1) Career\n2) Love\n3) Money\n4) Health" };
    }
    if (!f.topic) {
      const c = parseLeadingChoice(body.message);
      if (!c || c < 1 || c > 4) {
        return { handled: true, reply: lang === "ko" ? "주제를 번호로 골라줘.\n1) 커리어\n2) 연애\n3) 돈\n4) 건강" : "Choose a topic.\n1) Career\n2) Love\n3) Money\n4) Health" };
      }
  ...
  if (mode === "career") {
    ...
    if (!c.goal) {
      if (!choice || choice < 1 || choice > 4) {
        return { handled: true, reply: lang === "ko" ? "목표를 번호로 골라줘.\n1) 취업/지원\n2) 레주메\n3) 면접\n4) 포트폴리오" : "Choose a goal.\n1) Job search\n2) Resume\n3) Interviews\n4) Portfolio" };
      }
  ...
  if (mode === "love") {
    ...
    if (!l.stage) {
      if (!choice || choice < 1 || choice > 4) {
        return { handled: true, reply: lang === "ko" ? "상황을 번호로 골라줘.\n1) 썸/대화 중\n2) 연애 중\n3) 장기 관계\n4) 이별/정리 중" : "Choose a situation.\n1) Talking stage\n2) Dating\n3) Long-term\n4) Breakup / moving on" };
      }
...
chatRouter.post("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!state.mode) {
      ...
      if (!n || n < 1 || n > 5) return finish(() => sendPlainAssistant(res, askModeMenu(body.lang)));
    ...
    if (slot.handled) {
      ...
      if (!isReadyToCallLLM(state)) {
        const q = nextQuestionForMode(state.mode!, body.lang, state);
        return finish(() => sendPlainAssistant(res, q ?? (body.lang === "ko" ? "좋아. 계속 말해줘." : "Okay—go on.")));
      }
      // 슬롯 완성 → 아래 LLM 호출로 진행
    }
